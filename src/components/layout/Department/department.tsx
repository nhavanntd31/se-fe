...
import { getSemesters, getStatistic, GetStatisticInfoDto, StatisticResponse, getDepartments } from "@/services/statistic.service";
...
export default function Department() {
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      ...
      setDepartments(data);
      if (data.length > 0 && selectedDepartment === DEFAULT_DEPARTMENT) {
  ...
  const getDepartmentPickerItems = () => {
    return [
      { label: "Tất cả khoa", value: DEFAULT_DEPARTMENT },
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
        <div className="flex items-center justify-between">
          <h2 className="text-3xl font-bold tracking-tight">Khoa</h2>
          <div className="flex items-center space-x-2">
            <DateRangePicker
              items={getDepartmentPickerItems()}
        ...
        <div className="grid ga p-4 md:grid-cols-2 lg:grid-cols-2">
          <CPAAnalysisCard 
            ...
            scope={selectedDepartment === DEFAULT_DEPARTMENT ? "Trường" : "Khoa"}
          />
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    {departments.map((department) => (
                      <DropdownMenuItem 
                        key={department.id} 
                        onClick={() => handleOpenDialog(department.name)}
                      >
                        {department.name}
                      </DropdownMenuItem>
                    ))}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-full">
          <DialogHeader>
            <DialogTitle>So sánh với {selectedMajor}</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-8">
            <div className="space-y-4">
              <h3 className="text-lg font-medium">Khoa hiện tại</h3>
              <div className="grid grid-cols-2 gap-4">
                <StatCard
                  title="Tổng số lượng sinh viên"
                  value={statistics?.totalStudents?.toString() || "0"}
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-medium">{selectedMajor}</h3>
              <div className="grid grid-cols-2 gap-4">
                <StatCard
                  title="Tổng số lượng sinh viên"
                  value={statistics?.totalStudents?.toString() || "0"}
